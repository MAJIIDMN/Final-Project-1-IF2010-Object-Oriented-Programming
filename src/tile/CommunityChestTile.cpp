...
void CommunityChestTile::onLand(Player& player, GameContext& ctx, int diceTotal) {
	if (!ctx.cardSystem) return;
	auto card = ctx.cardSystem->drawCommunityChest();
	if (!card) return;
	...
	if (resolvePeerPaymentCard(rawResult)) {
		ctx.cardSystem->discardCommunityChest(std::move(card));
		return;
	}
